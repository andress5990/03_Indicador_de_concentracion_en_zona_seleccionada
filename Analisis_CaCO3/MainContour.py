...
def gauss(model, x, y, p0):
    ...
    maxg = popt[0]#Valor del maximo del ajuste gaussiano
    meang = popt[1]#Valor del promedio del ajuste guassiano
    sigmag = popt[2]#Calculo del sigma del ajuste gaussiano
...
while(1):
    ...
    if k == 32: #space
        ...
        #Analisis
        x = np.linspace(1.0, len(hist), len(hist)) #creamos los x para hace el fit, (inicio, final, numero total)
        ...
        #pdb.set_trace()
        Tcount = Countsum(hist)
        print("Las cuentas totales distintas de cero son: " + str(Tcount))
